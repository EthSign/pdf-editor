...
function datepicker(selectorOrElement, options) {
  ...
  if (instance.shadowDom) {
    ...
    if (!shadowDomAlreadyInUse) {
      applyListeners(instance.shadowDom);
    }
  }
  ...
  */
  if (instance.second) {
    ...
    adjustDateranges({ instance, deselect: !instance.dateSelected });
...
function createInstance(selectorOrElement, opts) {
  ...
  */
  ...
  const somePickerHasInlinePosition = pickersWithSamePositionedEl.some(
    function (picker) {
...
function sanitizeOptions(opts) {
  ...
  */
  const positionFound = ["tr", "tl", "br", "bl", "c"].includes(position);
  ...
  [
    ...
    if (typeof options[fxn] !== "function") {
      options[fxn] = noop;
    } // `noop` defined at the top.
  });
  ...
  */
  ...
  if (typeof overlayPlaceholder !== "string") {
    delete options.overlayPlaceholder;
  }
  if (typeof overlayButton !== "string") {
    delete options.overlayButton;
  }
  ...
  */
  for (let i = 1; i <= totalSquares; i++) {
    ...
    */
    ...
    if (hasEvent && !isEmpty) {
      className += " qs-event";
    } // Don't show events on empty squares.
    if (outsideOfCurrentMonth) {
      className += " qs-outside-current-month";
    }
    if (showAllDates || !outsideOfCurrentMonth) {
      className += " qs-num";
    }
    if (isSelected) {
      className += " qs-active";
    ...
    if (isDisabled && !isEmpty) {
      className += " qs-disabled";
    } // Empty dates don't need the class name.
    if (isToday) {
      className += " qs-current";
    }
    if (isRangeStart && end && rangeIsNotSingleDay) {
      className += " qs-range-start";
    }
    if (isRangeMiddle) {
      className += " qs-range-middle";
    }
...
function createOverlay(instance, overlayOpen) {
  ...
  const shortMonths = overlayMonths
    .map(function (m, i) {
      return (
...
function selectDay(target, instance, deselect) {
  ...
  if ((el.disabled || el.readOnly) && instance.respectDisabledReadOnly) {
    return;
  }
...
function calculatePosition(instance) {
  ...
  */
  if (centered) {
    return instance.calendarContainer.classList.add("qs-centered");
  }
...
function oneHandler(e) {
  ...
  */
  ...
  if (instance && instance.isMobile && instance.disableMobile) {
    return;
  }
  ...
  if (type === "click") {
    ...
    if (instance.noPosition && !onCal) {
      ...
      */
      if (monthDirection) {
        ...
        */
        const newDays = instance.calendar.querySelectorAll(
          '[data-direction="0"]'
        );
        let newTarget;
        let idx = 0;
   ...
   // hideCal(instance);
    ...
    if (!instance || !instance.calendar.contains(target)) {
      return;
    }
...
function remove() {
  ...
  */
  ...
  if (!datepickers.length) {
    removeEvents(document, oneHandler);
  }
